        ...
        if (!hasError) {
            ...
            registrationTask.setOnSucceeded(event -> {
                ...
                if ("username_taken".equals(message)) {
                    ...
                    try {
                        int userId = DatabaseUtility.getUserId(username);
                        Session.setSession(userId, username);
                        System.out.println("✅ Session set after registration - User: " + username + ", ID: " + userId);
                    } catch (Exception ex) {
                        System.out.println("❌ ERROR setting session: " + ex.getMessage());
                        ex.printStackTrace();
                    }
